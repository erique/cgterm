...
/* AC */
#ifdef __amigaos4__
 const char *version = "\0$VER: CGEdit 1.6 (28.05.2005) Port by Alex Carmona";
#endif
/* ac */
...
/* AC */
#elif __amigaos4__
  if (cfg_readconfig("PROGDIR:data/cgedit.cfg") < 0) {
    return(1);
  }
  if (cfg_read == 0) {
    cfg_writeconfig(default_cgedit_cfg, "PROGDIR:data/cgedit.cfg");
    if (cfg_readconfig("PROGDIR:data/cgedit.cfg") < 0) {
      return(1);
    }
  }
