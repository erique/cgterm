...
/* AC */
#ifdef __amiga_clib2__
  if (sscanf(line, "bookmark = %[^,] , %[ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.-] , %d \n", alias, hostname, &port) == 3) {
  } else if (sscanf(line, "bookmark = %[ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.-] , %d \n", hostname, &port) == 2) {
    strcpy(alias, hostname);
  } else if (sscanf(line, "bookmark = %[^,] , %[ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.-] \n", alias, hostname) == 2) {
  } else if (sscanf(line, "bookmark = %[ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.-] \n", hostname) == 1) {
    strcpy(alias, hostname);
  } else {
    return(0);
